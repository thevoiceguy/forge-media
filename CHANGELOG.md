...
**Shedding before slowing, and fuzzing** (FCP video conferencing, phase 6c;
released without this entry, written after the fact). `forge-video` 0.4.0:
`LoadShedder` and `VideoClock::done_with`, so a room gives up an output's
rung — the largest first, then the next — before it halves everyone's frame
rate. `forge-conference` 0.11.0: `RoomShedder`, `VideoRoomEvent::Shed
{ output, from, to }` in both directions, `forge_conference_video_shed_total`.
Eight `cargo-fuzz` targets under `fuzz/` (the five depacketizers, the frame
assembler, compound RTCP, the WebM reader) on corpora derived by the
`fuzz_seeds` tests in forge-rtp and forge-webm, run nightly.

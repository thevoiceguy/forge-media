...
async fn create_session(
    ...
    let sdp_negotiation_result = if let Some(ref offer_text) = request.sdp_offer {
        ...
        counter!("forge_sdp_negotiation_total").increment(1);

        // Validate that local_address is provided
        let local_addr = request.local_address.as_ref().ok_or_else(|| {
            counter!("forge_sdp_negotiation_failures_total", "reason" => "missing_local_address")
                .increment(1);
        ...
        let profile = match profile_name {
            ...
            _ => {
                counter!("forge_sdp_negotiation_failures_total", "reason" => "invalid_profile")
                    .increment(1);
        ...
        let codec_config = if let Some(audio_media) = answer.find_media(forge_sdp::MediaType::Audio)
        {
            ...
            if !codec_names.is_empty() {
                ...
                for codec_name in &codec_names {
                    counter!("forge_sdp_codecs_negotiated_total", "codec" => canonical_codec_label(codec_name)).increment(1);
                }
            }

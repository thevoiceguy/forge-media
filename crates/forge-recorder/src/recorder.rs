...
impl AudioRecorder {
    ...
    pub fn start(&self) -> Result<()> {
        ...
        let writer = match self.format.codec {
            ...
            AudioCodec::Opus => {
                ...
                let frame_size =
                    (self.format.sample_rate / 50) as usize * self.format.channels as usize;
                ...
                RecorderWriter::Opus {
                    ...
                    while !remaining.is_empty() {
                        ...
                        if frame_buffer.len() == *frame_size {
                            self.encode_opus_frame(encoder, frame_buffer, ogg_stream)?;
                            frame_buffer.clear();
                        }
                    }
                }
    ...
    fn encode_opus_frame(
        ...
        ogg_stream
            .write_packet(packet, 0, ogg::PacketWriteEndInfo::NormalPacket, None)
            ...
            match writer {
                ...
                RecorderWriter::Opus {
                    ...
                    if !frame_buffer.is_empty() {
                        ...
                        self.encode_opus_frame(&mut encoder, &padded_frame, &mut ogg_stream)?;
                    }
